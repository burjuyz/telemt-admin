    ...
    match action {
        ...
        CallbackAction::ShowUserSelectionActions => {
            ...
            ack_callback(
                bot,
                q.id.clone(),
                Some(&format!("Выбрано: {} пользователей", count)),
                false,
            )
            .await?;
        ...
        CallbackAction::BulkAssignGroup { group_id } => {
            ...
            let group_name = group
                .map(|g| g.name)
                .unwrap_or_else(|| "группы".to_string());
            {
        ...
        CallbackAction::BulkBanUsers => {
            ...
            ack_callback(
                bot,
                q.id.clone(),
                Some(&format!("Заблокировано: {} пользователей", count)),
                false,
            )
            .await?;
            admin_show_users_page(bot, chat_id, state, 1, Some(message_id)).await?;
        ...
        CallbackAction::BulkSetUserLimit { field } => {
            ...
            let field_name = match field {
                ...
                crate::bot::handlers::callback_data::UserLimitField::DataQuotaBytes => {
                    "квота трафика"
                }
        ...
        CallbackAction::ExportUsersCsv => {
            ...
            ack_callback(
                bot,
                q.id.clone(),
                Some(&format!("Экспорт {} пользователей", count)),
                false,
            )
            .await?;
            let filename = format!(
            ...
            bot.send_document(
                chat_id,
                InputFile::memory(csv.into_bytes()).file_name(filename),
            )
            .await?;
            Ok(true)
        }
