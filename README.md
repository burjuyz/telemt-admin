...
1. Заполните `.env` секретами: как минимум `TELEMT_ADMIN__BOT_TOKEN` и `TELEMT_ADMIN__TELEMT_API__AUTH_HEADER`.
...
- `🪄 Deep link` — отправляет deep link для быстрого открытия карточки токена через `/start`.
